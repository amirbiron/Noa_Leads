...
> **הערה (המשך):** ה-`daily_summary` הסטטיסטי הזה (ספירות) מתוכנן להחלפה בסיכומי AI נרטיביים כחלק מ-C.1/C.2 — ראה `docs/c1-c2-summaries-spec.md` (בתכנון, טרם מומש). זה רמז להמשך בלבד; סטטוס ה-finding לא משתנה.
...
### F-24: waiting_on של 2 chips ב-§16.4 שגוי — 🟢 הושלם

**Spec v2.1 §16.4 (לפני התיקון):**
- "מעוניין בשיחה" → waiting_on=CLIENT
- "לחזור בעוד חודש" → waiting_on=CLIENT

**הכרעת משתמש (בדיקה ידנית):** במציאות הכדור אצל נועה — היא צריכה לתאם / לחזור, לא להמתין שהלקוח ייצור קשר. ה-spec שגוי בנקודה הזו.

**Code:**
- `backend/alembic/versions/2026_05_24_0016-0016_chip_waiting_on_fix.py` — UPDATE שני ה-chips ל-NOAH ב-DB קיים.
- `backend/alembic/versions/2026_05_24_0013-0013_chips_schema_v21.py` — seed dict מתוקן ל-clean installs.
- `docs/SpecV2.1.md` §16.4 + Changelog v2.1 מעודכנים.

**Severity:** בינוני (UX — מנהל ציפיות שגוי לנועה: ליד היה נראה "אצל הלקוח" כשהיא הייתה צריכה לפעול).

**הערה:** "לא רלוונטי כרגע" נשאר waiting_on=CLIENT — באמת הלקוח אמור לחזור אם המצב משתנה. ה-dormant_check task ב-60 יום מטפל באפס תגובה.

**Acceptance:**
- [x] migration 0016 מתקן 2 chips ב-DB קיים.
- [x] seed ב-0013 מתוקן (consistent עם 0016 אחרי שתיהן רצו).
- [x] Spec §16.4 + Changelog מעודכנים.
...
**Skill:** `docs/Skills/israeli-phone-formatter/SKILL.md` שורה 52 מחדד: "הצורה עם המקפים היא **לתצוגה לבני אדם בלבד**. אם תעבירו אותה ל-parser מחמיר של E.164 (`google-libphonenumber`, WhatsApp Business API, AWS SNS) הוא ידחה אותה. **אחסנו את הצורה הקנונית בלי מפרידים, ורק בתצוגה הוסיפו מקפים.**"

**Code:** `backend/app/utils/phone.py` עושה את **ההפך** — `_clean()` ממיר `+972`/`972` ל-`0` מוביל, ו-`normalize_for_storage()` מחזיר את צורת התצוגה עם מקפים (`052-1234567`). כלומר ה-DB מחזיק את צורת התצוגה, וההמרה ל-E.164 נדחית ל-frontend (`frontend/lib/phone.ts::toWhatsAppDigits`) בכל פעם שבונים קישור וואטסאפ.

**Severity:** בינוני — לא שובר כלום היום (ה-frontend ממיר בעצמו), אבל כל צרכן עתידי שיקרא את העמודה ישירות (WhatsApp Business API, SMS, ייצוא) יקבל ערך שלא עומד ב-E.164.

**Acceptance:**
- [ ] `normalize_for_storage` מחזיר ספרות בלבד בפורמט קנוני
- [ ] פונקציית תצוגה נפרדת מוסיפה מקפים בכל מקום שמציג טלפון
- [ ] מיגרציית נתונים להמרת `leads.phone` ו-`bookings.contact_phone` הקיימים
- [ ] `frontend/lib/phone.ts::toWhatsAppDigits` מפושט בהתאם

**למה לא תוקן בסבב הזה:** הסטייה נוגעת ל**כל** הלידים במערכת ולכל מסלולי התצוגה, לא רק לפגישות. תיקון דורש מיגרציית נתונים ובדיקה של כל צרכן. בסבב הזה הוקפד רק שהשדה החדש `bookings.contact_phone` ישתמש ב**אותה** פונקציה כמו `leads.phone` (`normalize_phone_input`), כדי שלא ייווצר פורמט שלישי.
...
**Code:** `frontend/app/leads/[id]/page.tsx` — הקומפוננטה `CopyBookingLinkButton` מציגה "העתקת קישור לקביעת פגישה" בכרטיס הליד ומעתיקה `${origin}/book/{booking_token}` ללוח.

**Severity:** מינורי — הפיצ'ר עובד; רק התיעוד חסר.

**הערה מהסבב הזה:** הלקוח דיווח על "שגיאה בהוצאת קישור פעם שנייה". בבדיקה התברר שהכפתור הוא clipboard בלבד ואינו פונה לשרת כלל — מה שנראה כשגיאה היה מסך החסימה בדף הציבורי עצמו ("כבר יש לך בקשת פגישה"), שהוסר ב-§11.1. **אין באג בכפתור.**

**Acceptance:**
- [ ] §11.1 מתאר את הכפתור, את מבנה ה-URL, ואת העובדה שהקישור קבוע וניתן לשימוש חוזר
- [x] `booking_token` מתועד ב-§5.1
...
- **v1.3 (ספטמבר 2026):** סבב ביטול שלב האישור.
  ...
  - **F-33 חדש** — סתירה בין §25.2 (`+972`) לבין הפורמט המקומי שנשמר בפועל.
  - **F-34 חדש** — כפתור העתקת הקישור לא מתועד; כולל בירור של "הבאג" שדווח עליו.
  - **באג שתוקן ולא היה מתועד כ-finding:** `approve_meeting` ב-state machine העביר ליד ל-`BOOKED` בלי ליצור אירוע ביומן ובלי לגעת בשורת ה-Booking — שינוי סטטוס בלי ה-side-effects שלו (כלל 13). הפעולה נמחקה.
  - **ממצא בתשתית הבדיקות:** ה-fixture ב-`conftest.py` בלע שגיאת event-loop ודיווח עליה כ"אין DB". בפועל רק בדיקת DB אחת רצה בכל קובץ, ו-54 בדיקות דולגו בשקט. אחרי התיקון: 179 עוברות, אפס דילוגים.
