...
- **תיאור:** מפתח חתימה ל-JWT tokens. **חייב להיות חזק וייחודי בפרודקשן**
  ...
  **עדיין נדרש גם אחרי ביטול מסך ההתחברות** (Spec §22.1.1): הכניסה
  האוטומטית מנפיקה בדיוק אותם JWT tokens, והם גם מה שנושא את
  ה-`state` ב-OAuth של Google. **החלפת הערך מנתקת את כל הדפדפנים
  הפתוחים** — הם יקבלו 401, ייכנסו אוטומטית מחדש, ויתאוששו לבד.
