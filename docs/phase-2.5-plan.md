...
### 3.2 אייקונים "אצלי"/"אצל הלקוח"

לפי ה-spec (שורה 148-152):
- מחכה לי (נועה) = ברירת מחדל, אין סימון
- מחכה ללקוח = אייקון ⏳ קטן ליד שם הליד

**אבל המשתמש ביקש**: "אצלי - ⏳, אצל הלקוח - 📨". זה הפוך מ-spec!

**❓ החלטה נדרשת:** מה האייקון לכל מצב?

### 3.4 "שלחי הצעה" — הפרדת תיעוד משליחה אמיתית

**הבעיה:** הכפתור מעדכן `PROPOSAL_SENT` אבל לא באמת שולח הצעה — אין עדיין חיבור למייל/וואטסאפ.

**אופציות:**
- **A:** שינוי הטקסט ל"סמני שנשלחה הצעה" — מבהיר שזה תיעוד ידני. נועה שולחת בנפרד.
- **B:** שני שלבים: "פתח טיוטת הצעה" (פותח WhatsApp/email עם תבנית), אחר כך "סמני שנשלח".
- **C:** לדחות עד שיש תבנית הצעה אמיתית (פאזה 3?).

**❓ החלטה נדרשת:** איזו גישה?
...
לפני שמתחילים, צריך תשובות על:

1. **§3.2** — מה האייקון לכל מצב waiting_on? (Spec אומר A, המשתמש ביקש B)
2. **§3.4** — איזה approach ל-"שלחי הצעה"? (A/B/C)
3. **§4.1** — לאן להוסיף את `next_action`: לטבלת `leads` או דרך task בלבד?
4. **§1.2** — האם להוסיף כללי פולואפ per-type עכשיו, או להמתין לפיצ'ר settings?
5. **§5** — האם להוסיף שדה `target_audience` ל-templates אם לא קיים, או להחליף עם תיוג אחר?
