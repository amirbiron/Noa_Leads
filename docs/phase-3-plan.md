...
> **⚠️ superseded:** התיאור כאן הוחלף. האפיון המלא, העדכני, וה-source of truth לפיצ'ר הזה הוא **`docs/c1-c2-summaries-spec.md`** (C.1 יומי + C.2 שבועי). בפרט, ההערה למטה על "ההודעה בטלגרם הופכת ל-dynamic" **בטלה** — לפי האפיון החדש הסיכומים מוצגים בעמוד "היום" בלבד, **בלי טלגרם ובלי מייל**. הסעיף הזה נשאר לצרכי היסטוריה.
