...
def after_hours_reply_message(dt: datetime) -> str:
    ...
    מבדילה בין סוף שבוע רגיל לבין חג, לפי האפיון.
    """
    local = to_israel_tz(dt)
    d = local.date()

    # אם זה חג, ערב חג, או שבת — ההודעה הכללית של "יום העבודה הקרוב"
    if is_holiday(d) or is_holiday_eve(d) or is_saturday(d):
        return (
            "קיבלנו את הפנייה.\n"
            "נחזור אליך ביום העבודה הקרוב בין 9:00 ל-11:00."
        )

    # ימי שישי אחרי 16:00 — מחזירים ב"ראשון" (אלא אם ראשון עצמו חג)
    return (
        "קיבלנו את הפנייה.\n"
