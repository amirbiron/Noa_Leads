...
לגבי full_name — חפש לפי הסדר הבא:
1. בחתימה בסוף המייל ("תודה, אמיר" / "בברכה, יוסי כהן" / "בכבוד רב, מירב").
2. בהצגה עצמית בגוף המייל ("שלום, אני מירב מ-XYZ").
אם לא נמצא שם בטקסט — החזר null (אל תמציא שם פיקטיבי). המערכת תשלים את \
השם מכותרת ה-From או מה-domain בעצמה.
