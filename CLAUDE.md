...
| `docs/c1-c2-summaries-spec.md` | **המקור היחיד והאמיתי לפיצ'ר C.1+C.2** (סיכומים יומיים ושבועיים מבוססי-AI) — **בתכנון, טרם מומש**. מחליף את התיאור הישן של "חבילה C" ב-`phase-3-plan.md`. מכסה: קלט מבושל (AI מפרש לא מחשב), פרומפטים מלאים + few-shot, output schema, טבלת `ai_summaries`, ולידציה, cron, ו-UI ב-/today (בלי טלגרם). **לקרוא במלואו לפני מימוש C.1/C.2.** |
...
- כותב `except` שבולע — `pass`, ערך falsy, או המשך כאילו הצליח → לוג, או הערה שאומרת למה הכשל הזה אינו מעניין. בלעדיהם זה ממצא, גם כשאין ✅ למשתמש במסלול. ‏(`raise` מחדש אינו בליעה)
...
**מה שבמכוון אין לו שורה:** תווי BiDi בהערות עברית. `CLAUDE.md` מחייב הערות בעברית, ולכן שורה כזו הייתה נדלקת על כל שינוי בכל קובץ — ושורה שחלה תמיד מלמדת לדלג על הטבלה. הכלל נאכף ב-`backend/tests/test_no_bidi_controls.py`.
